...
<table cellspacing="0" cellpadding="0">
    ...
    if ($result = mysqli_query($link, $query)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <tr>
                <td>Описание ошибки ФЛК</td>
                <td><font color=red><?php echo $row['error_text']; ?></font></td>
            </tr>
            <tr>
                <td>Значение элемента</td>
                <td><?php echo $row['error_value']; ?></td>
            </tr>
            <tr>
                <td>Значение элемента</td>
                <td><?php echo $row['error_poz']; ?></td>
            </tr>
            <tr>
                <td>Файл выгрузки</td>
                <td><?php echo $row['file_urr_xml']; ?></td>
            </tr>
            <tr>
                <td>Файл протокола ФНС</td>
                <td><?php echo $row['idfile_fns_xml']; ?></td>
            </tr>
...
<form name="records_note" action="?" method="get">
	...
	rnf.record_list_id=$record_list_id";
        ...
        ?>
        <tr>
            <td width="30%">Решение по исправлению</td>
            <td>
                <input type="radio" name="decision_type"
                       value=0 <?php if (!isset($row2['decision_type']) or $row2['decision_type'] == 0) echo 'checked'; ?> >Не
                обработана<br>
                <input type="radio" name="decision_type"
                       value=1 <?php if (isset($row2['decision_type']) and $row2['decision_type'] == 1) echo 'checked'; ?> >В
                работе<br>
                <input type="radio" name="decision_type"
                       value=2 <?php if (isset($row2['decision_type']) and $row2['decision_type'] == 2) echo 'checked'; ?> >Исправлена<br>
                <input type="radio" name="decision_type"
                       value=3 <?php if (isset($row2['decision_type']) and $row2['decision_type'] == 3) echo 'checked'; ?> >Невозможно
                исправить<br>
                <input type="radio" name="decision_type"
                       value=4 <?php if (isset($row2['decision_type']) and $row2['decision_type'] == 4) echo 'checked'; ?> >Не
                обнаружена<br>
            </td>
        </tr>
        <input type="hidden" name="record_notes_id" value="<?php //$row2[id]?>"/>
        <tr>
            <td width="30%">Номер заявки на техошибку в ЕГРН</td>
            <td><textarea name="reg_no" type="text" rows="1" cols="100"><?php echo $row2['reg_no']; ?></textarea></td>
        </tr>
        <tr>
            <td width="30%">Комментарий по устранению ошибки - ФИО исполнителя</td>
            <td><textarea name="text" type="text" rows="10" cols="100"><?= $row2['text']; ?></textarea></td>
        </tr>
        <?php
    ...
    <br>
    <button class="button" name="records_note_submit" type="submit" value="save">Сохранить</button>&#160;
