...
if ($search_period_submit == "1") {
    if (isset($s_type_unloading) and $s_type_unloading >= "0") {
        if (!empty($s_period_start) or !empty($s_period_stop)) {
            $s_tu_where = " AND `type` = '" . $s_type_unloading . "'";
            $f_tu_string = " по типу выгрузки: вкл.";
        } else {
            $s_tu_where = "WHERE `type` = '" . $s_type_unloading . "'";
            $f_tu_string = " по типу выгрузки: вкл.";
        }
    } else {
        $s_tu_where = "";
...
$query = 'SELECT 
		...
		LEFT JOIN protokol_file pf ON pe.id=pf.protokol_id ' . $s_where . '
...
<table class="main">
    ...
    <tr>
        <form enctype="multipart/form-data" method="POST" name="flk_donwload" action="">
            ...
            <th class="main">
                <input required type="date" class="date w130" name="period_start" max="<?= $curr_date; ?>"
                       value="<?= $period_start; ?>"/>
            </th>
            <th class="main"><input required type="date" class="date w130" name="period_stop" max="<?= $curr_date; ?>"
                                    value="<?= $period_stop; ?>"/>
            </th>
            <th class="main">
    ...
    if ($result = mysqli_query($link, $query)) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            */
            ...
            foreach ($arr_vid_object as $id_vid_object => $name_vid_object) {
                if ($row['vid_object'] == $id_vid_object) {
                    break;
            ...
            echo '<tr class="hover" title="Загружен ' . $date . '">
					<form name="flk_edit' . $k . '" method="GET" action="?">
					<input type="hidden" name="id" value="' . $row['id'] . '" />
					<input type="hidden" name="protokol_uid" value="' . $row['protokol_id'] . '" />
					<!-- 
					<td class="main"> 
					...
					<td class="main filelst">' . $row['file_name_excel'] . '</td>
