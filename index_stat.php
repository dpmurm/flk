...
<div class="flk-export">
   ...
   // echo '<th>' . ' ' . '</th>';
    ...
    foreach ($model as $item) {
        print '<tr>';
        //print '<td>'.($item->id).'</td>';
        echo '<td><a href="flk_protokol_stat_procent_export.php?protokol_id='.$item->id.'&number='.$item->number.'&year='.$item->Year.'&period_start='.$item->period_start.'&period_stop='.$item->period_stop.'">
             % прохождения</a></td>';
        echo '<td><a href="flk_protokol_stat_procent_ispravlen.php?protokol_id='.$item->id.'&number='.$item->number.'&year='.$item->Year.'&period_start='.$item->period_start.'&period_stop='.$item->period_stop.'">
             ход исправления</a></td>';
        echo '<td><a href="flk_protokol_stat_vid_oshibok.php?protokol_id='.$item->id.'&number='.$item->number.'&year='.$item->Year.'&period_start='.$item->period_start.'&period_stop='.$item->period_stop.'">
             виды ошибок</a></td>';
