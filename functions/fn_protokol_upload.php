...
function flk_protokol_add($link, $arr_xls_heads)
{
	...
	if (file_exists($tmp_file_xls) and isset($file_xml) and $type_file_xls == 'application/vnd.ms-excel' and $type_file_xml == 'text/xml') 
	{
		...
		$query_check_filename_xls = "SELECT file_name_xls FROM protokol_export WHERE `file_name_xls` = '".$file_xls."' LIMIT 1";
		$result_check_filename_xls = mysqli_query($link, $query_check_filename_xls);
	
		$query_check_filename_xml = "SELECT file_name_xml FROM protokol_export WHERE `file_name_xml` = '".$file_xml."' LIMIT 1";
...
function flk_protokol_parsing($link, $arr_xls_heads, $date, $period_start, $period_stop, $visible, $type_unloading, $vid_object, $protokol_uid, $file_xls, $tmp_file_xls, $file_xml)
{
	...
	if(isset($link, $arr_xls_heads, $date, $period_start, $period_stop, $visible, $type_unloading, $vid_object, $protokol_uid, $file_xls, $tmp_file_xls, $file_xml))
	{
		...
		*/
		...
		$query_add_prot = "INSERT INTO protokol_export (`insert_date`, `date`,`period_start`,
			`period_stop`,`visible`,
			`type_unloading`,`vid_object`,
			`file_name_xls`,`file_name_xml`,`protokol_uid`) 
		VALUES (CURDATE(), '".$date."', '".$period_start."', 
			'".$period_stop."', '".$visible."', 
			'".$type_unloading."', '".$vid_object."', 
			'".$file_xls."', '".$file_xml."', 
			'".$protokol_uid."')";

		mysqli_query($link, $query_add_prot) or die ("Error in query: ".$query."<br>".mysqli_error($link));
